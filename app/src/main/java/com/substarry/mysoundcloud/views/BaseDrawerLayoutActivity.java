...
public abstract class BaseDrawerLayoutActivity extends BaseSwipeRefreshLayoutActivity {
     ...
     *  如果要使用 menuItemChecked， 务必返回正确的items id
     * @return int[]
     */
    protected abstract int[] getMenuItemIds();

    /**
     * 被 menuItemChecked 调用的，真正实现点击的地方
    ...
    /**
     * avoid select same item twice
     * 通过checked状态的保存，避免连续两次点击同一个item时，重复click
     * 当确认不是连续两次点击时，会调用 onMenuItemOnClick，去实际完成点击操作
     * @param itemId itemId
